...
export function resolveMovieVisuals(input: {
  ...
  return {
    poster: buildTMDBImageUrl(input.poster ?? input.poster_path ?? input.image ?? input.poster_url),
    backdrop: buildTMDBImageUrl(input.backdrop ?? input.backdrop_path, 'w1280'),
  };
}
