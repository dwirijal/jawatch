...
import { SHORTS_BACK_LABEL, SHORTS_HUB_HREF, getShortsEpisodeHref } from '@/lib/shorts-paths';
import { getVerticalDramaChunkIndex, getVerticalDramaProgress } from '@/lib/vertical-drama-store';
...
export default function DrachinDetailClient({ slug, browseHref = SHORTS_HUB_HREF }: DrachinDetailClientProps) {
  const [detail, setDetail] = React.useState<DrachinDetailData | null>(null);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);
  const [playHref, setPlayHref] = React.useState(getShortsEpisodeHref(slug, slug, 1));
  ...
  React.useEffect(() => {
    ...
    getDrachinDetailBySlug(slug)
      .then((nextData) => {
        if (!cancelled) {
          setDetail(nextData);
          setError(nextData ? null : 'Detail cerita belum tersedia untuk judul ini.');
        }
      })
      .catch((cause) => {
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : 'Detail cerita belum bisa dimuat.');
        }
      })
      .finally(() => {
        if (!cancelled) {
  ...
  const syncResumeTarget = React.useCallback(() => {
    ...
    const storedEpisodeIndex = resume?.episodeIndex ?? 1;
    const fallbackEpisode =
      detail?.episodes.find((item) => Number.parseInt(item.index, 10) === storedEpisodeIndex) ??
      detail?.episodes[Math.max(0, Math.min((detail?.episodes.length ?? 1) - 1, storedEpisodeIndex - 1))] ??
      null;
    const episodeSlug = fallbackEpisode?.slug ?? slug;
    const episodeIndex = Number.parseInt(fallbackEpisode?.index ?? String(storedEpisodeIndex), 10) || storedEpisodeIndex;

    setPlayHref(getShortsEpisodeHref(slug, episodeSlug, episodeIndex));
    setResumeEpisodeIndex(episodeIndex);
  }, [detail, slug]);
  ...
  const resumeLabel = resumeEpisodeIndex > 1 ? `Lanjut episode ${resumeEpisodeIndex}` : 'Mulai episode 1';

  return (
    <VerticalSeriesDetailScaffold backHref={browseHref} backLabel={SHORTS_BACK_LABEL}>
        {loading ? (
          ...
          <StateInfo type="error" title="Detail cerita belum bisa dibuka" description={error || 'Cerita yang kamu cari belum bisa dimuat.'} />
        ) : (
          <>
                ...
                <div className="space-y-4 md:space-y-5">
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    {resumeEpisodeIndex > 1 ? (
                      <Badge variant="outline">Terakhir ditonton: EP {resumeEpisodeIndex}</Badge>
                    ) : null}
                  </div>
                </div>
              </div>
            ...
            <section className="space-y-4">
              <SectionHeader
                ...
                subtitle={`${detail.totalEpisodes} episode pendek tersedia. Daftar dibuka dari bagian yang sesuai dengan tontonan terakhir kamu.`}
                icon={Clapperboard}
              />
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                {activeChunk?.items.map((episode) => {
                  ...
                  <Paper key={`${episode.slug}:${episode.index}`} asChild tone="muted" shadow="sm" padded={false}>
                    <Link
                      href={getShortsEpisodeHref(slug, episode.slug, episode.index)}
