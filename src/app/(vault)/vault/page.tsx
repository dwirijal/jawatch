...
import { CollectionSections } from '@/features/vault/CollectionSections';
import { DeferredAdSection } from '@/components/organisms/DeferredAdSection';
import { VaultCommunitySummary } from '@/components/organisms/CommunityPanel';
...
export const metadata: Metadata = {
  title: 'Koleksi',
  description: 'Simpanan, riwayat, dan pengaturan akun kamu di jawatch.',
  robots: {
    index: false,
    follow: false,
  },
      ...
      <div className="app-container space-y-8 py-4 sm:py-6">
        <section className="surface-panel-elevated p-6 sm:p-8">
          ...
          <p className="max-w-2xl text-sm leading-6 text-zinc-500">
            Simpanan, riwayat nonton, riwayat baca, dan pengaturan akun ada di sini.
          </p>

          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              { href: '/vault/profile', label: 'Profil', icon: UserRound, copy: 'Identitas, akses umur, tema, dan bahasa.' },
              { href: '/vault/history', label: 'Riwayat', icon: History, copy: 'Progres nonton dan baca terbaru.' },
              { href: '/vault/saved', label: 'Simpanan', icon: Bookmark, copy: 'Bookmark dan favorit kamu.' },
            ].map((item) => (
              <Link
                key={item.href}
                href={item.href}
