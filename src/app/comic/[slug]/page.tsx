      ...
      <div className="app-shell flex min-h-screen items-center justify-center p-6">
        <Paper tone="muted" shadow="sm" className="flex max-w-md flex-col items-center gap-4 text-center">
          ...
          <Button variant="manga" asChild>
            <Link href="/comic">Kembali ke Library</Link>
          </Button>
        </Paper>
      </div>
    );
        ...
        hero={
          <section className="surface-panel-elevated relative overflow-hidden">
            ...
            <div className="relative z-10 px-5 py-5 md:px-6 md:py-6">
              <nav className="mb-6">
                <Link href="/comic" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.24em] text-muted-foreground transition-colors hover:text-foreground">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    {latestChapter ? (
                      <Button variant="manga" size="lg" asChild>
                        <Link href={`/comic/${slug}/${latestChapter.slug}`}>
                          Read Latest
          ...
          <>
            {latestChapter ? (
              <DetailActionCard
                theme="manga"
                title="Ready to read?"
                description="Halaman comic tetap metadata-first, tapi aksi membaca tetap dekat dan mudah dijangkau."
                actions={[
                  { href: `/comic/${slug}/${latestChapter.slug}`, label: 'Read Latest Chapter' },
                ...
                <div className="space-y-4">
                  {manga.similar_manga.map((similar) => (
                    <Paper key={similar.slug} tone="muted" shadow="sm" padded={false} asChild>
                      <Link href={`/comic/${similar.slug}`} className="group flex gap-4 p-4 transition-colors hover:bg-surface-elevated">
