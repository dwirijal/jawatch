...
export default async function NsfwSeriesWatchPage({ params }: PageProps) {
  const { slug } = await params;
  await requireNsfwAccess(getNsfwSeriesWatchHref(slug));
  ...
  return (
    <MediaWatchPage
      backHref={getNsfwSeriesDetailHref(episode.seriesSlug)}
      ...
      browseHref="/nsfw#series"
      browseLabel="More NSFW Series"
      theme={theme}
      stage={
        <VideoPlayer
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button variant="outline" disabled={!episode.prevEpisodeSlug} asChild={Boolean(episode.prevEpisodeSlug)}>
              {episode.prevEpisodeSlug ? (
                <Link href={getNsfwSeriesWatchHref(episode.prevEpisodeSlug)}>
            ...
            <Button variant="outline" disabled={!episode.nextEpisodeSlug} asChild={Boolean(episode.nextEpisodeSlug)}>
              {episode.nextEpisodeSlug ? (
                <Link href={getNsfwSeriesWatchHref(episode.nextEpisodeSlug)}>
      ...
      community={{
        mediaId: `nsfw:${episode.seriesSlug}`,
        title: episode.seriesTitle,
        type: 'series',
        theme,
      }}
