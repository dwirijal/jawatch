...
import MovieWatchHistoryTracker from '@/app/movies/watch/[slug]/MovieWatchHistoryTracker';
...
export default async function NsfwMovieWatchPage({ params }: PageProps) {
  const { slug } = await params;
  await requireNsfwAccess(getNsfwMovieWatchHref(slug));
  ...
  const routeHref = getNsfwMovieWatchHref(movie.slug);
  ...
  return (
    <MediaWatchPage
      ...
      backHref={getNsfwMovieDetailHref(movie.slug)}
      ...
      browseHref="/nsfw#movies"
      browseLabel="More NSFW Movies"
      theme="movie"
      stage={
              ...
              <div className="flex h-full flex-col items-center justify-center gap-3 p-5 text-center md:gap-4 md:p-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="mx-auto max-w-2xl text-sm leading-6 text-zinc-400">
                    Movie data sudah sinkron, tapi source saat ini belum menyediakan embed yang bisa diputar langsung di halaman.
                  </p>
                </div>
              </div>
            </div>
        ...
        <Paper tone="muted" shadow="sm" className="flex h-full min-h-0 flex-col gap-4 p-4 md:p-5">
          ...
          <div className="mt-auto flex flex-col gap-2.5">
            <Button variant="outline" className="h-11 w-full justify-center rounded-[var(--radius-md)] border-border-subtle bg-surface-1 hover:bg-surface-elevated" asChild>
              <Link href={getNsfwMovieDetailHref(movie.slug)}>
                View Details
