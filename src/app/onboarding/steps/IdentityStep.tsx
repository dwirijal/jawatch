export function IdentityStep({
  ...
  return (
    <form action={action} className="space-y-4">
      <div>
        <h2 className="text-sm font-black uppercase tracking-[0.16em] text-zinc-200">Identity</h2>
        <p className="mt-2 text-sm text-zinc-400">Set your display name.</p>
      </div>
      ...
      {errorCode === 'display-name' || errorCode === 'required' ? (
        <p className="rounded-[var(--radius-sm)] border border-red-400/40 bg-red-400/10 px-3 py-2 text-sm text-red-200">
          A display name is required before onboarding can be completed.
        </p>
      ) : null}
