export function TasteSeedsStep({
  ...
  return (
    <form action={action} className="space-y-4">
      <div>
        <h2 className="text-sm font-black uppercase tracking-[0.16em] text-zinc-200">Taste Seeds</h2>
        <p className="mt-2 text-sm text-zinc-400">
          Add a few genres and favorite titles so recommendations start stronger.
        </p>
      </div>

      {errorCode === 'taste' ? (
        <p className="rounded-[var(--radius-sm)] border border-red-400/40 bg-red-400/10 px-3 py-2 text-sm text-red-200">
          Could not save taste preferences. Try again.
        </p>
      ) : null}
